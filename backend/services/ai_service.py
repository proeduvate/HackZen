...
class AICoMentorService:
    ...
    async def generate_response(self, query: str, context: List[str], hackathon_id: str) -> Dict[str, Any]:
        """Generate AI response using RAG approach"""
        if not self.client:
            return {"response": "AI Co-Mentor is currently unavailable. Please try again later."}
        ...
        try:
            ...
            return {"response": f"I encountered an error: {str(e)}. Please try again or contact support."}
    
    async def _retrieve_relevant_documents(self, query: str, hackathon_id: str) -> List[str]:
        ...
        docs = await embeddings_collection.find({
    ...
    async def generate_idea_feedback(self, idea: str, hackathon_themes: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"feedback": "Unable to generate feedback at this time.", "error": str(e)}
    
    async def suggest_mentor_match(self, team_requirements: Dict[str, Any]) -> List[Dict[str, Any]]:
