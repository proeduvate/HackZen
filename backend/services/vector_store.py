...
class VectorStoreService:
    ...
    async def add_documents(
        self, hackathon_id: str, documents: List[Dict[str, Any]]
    ) -> int:
        ...
        try:
            ...
            embeddings = await loop.run_in_executor(
                self.executor, lambda: self.embedding_model.encode(texts).tolist()
            )
    ...
    async def search(
        self, query: str, hackathon_id: str, limit: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query_embedding = await loop.run_in_executor(
                self.executor, lambda: self.embedding_model.encode([query]).tolist()[0]
            )
    ...
    async def delete_hackathon_documents(self, hackathon_id: str) -> bool:
        ...
        try:
            ...
            results = self.collection.get(where={"hackathon_id": hackathon_id})
    ...
    async def get_hackathon_document_stats(self, hackathon_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not results or not results.get("documents"):
                return {"document_count": 0, "chunk_count": 0, "unique_documents": 0}
