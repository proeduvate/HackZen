...
class CertificateGenerator:
    ...
    def generate_certificate(self, certificate_data: Dict[str, Any]) -> bytes:
        ...
        # Subtitle
        story.append(Paragraph("This certificate is proudly presented to", self.subtitle_style))
        ...
        if certificate_data.get('team_name'):
            story.append(Paragraph(f"As part of team: <b>{certificate_data['team_name']}</b>", self.details_style))
        
        story.append(Paragraph(f"Role: <b>{certificate_data.get('role', 'Participant')}</b>", self.details_style))
        ...
        details_data = [
            ...
            ['Hackathon Duration:', f"{certificate_data.get('hackathon_duration', 'Multiple days')}"],
        ...
        # Verification text
        story.append(Paragraph("Scan QR code to verify this certificate", self.details_style))
        ...
        signatures_table = Table(signatures_data, colWidths=[2*inch, 2*inch, 2*inch])
    ...
    def create_certificate_data(self, user_data: Dict[str, Any], hackathon_data: Dict[str, Any], team_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        certificate_data = {
            ...
            'role': 'Team Lead' if team_data and team_data.get('team_lead_id') == user_data['id'] else 'Team Member' if team_data else 'Individual Participant',
            'status': 'issued'
        }
        ...
        if hackathon_data.get('hackathon_start') and hackathon_data.get('hackathon_end'):
            start = datetime.fromisoformat(hackathon_data['hackathon_start'].isoformat() if isinstance(hackathon_data['hackathon_start'], datetime) else hackathon_data['hackathon_start'])
            end = datetime.fromisoformat(hackathon_data['hackathon_end'].isoformat() if isinstance(hackathon_data['hackathon_end'], datetime) else hackathon_data['hackathon_end'])
            duration = (end - start).days
            certificate_data['hackathon_duration'] = f"{duration} day{'s' if duration != 1 else ''}"
