...
from schemas.team import TeamCreate, TeamUpdate, TeamMemberRole, TeamResponse, TeamMemberResponse, TeamMentorUpdate
...
async def create_team(
    ...
    hackathon = await db["hackathons"].find_one({"_id": ObjectId(team_data.hackathonId)})
    if not hackathon:
        raise HTTPException(
    ...
    stages = await stages_collection.find({"hackathonId": team_data.hackathonId}).sort("stageOrder", 1).to_list(1)
...
async def get_mentor_teams(current_user: dict = Depends(RequireRole(["mentor", "admin"]))):
...
async def join_team(team_id_or_code: str, current_user: dict = Depends(with_auth)):
    ...
    if not team:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Team not found with the provided ID or Code")
    ...
    current_members_count = await members_collection.count_documents({"teamId": str(team["_id"])})
...
async def assign_mentor(
    ...
    if team.get("mentorId"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Team already has a mentor assigned")
    ...
    if not membership:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the team leader can assign a mentor")
    
    # Verify the mentor exists
    mentor = await db["mentors"].find_one({"userId": mentor_data.mentorId})
    if not mentor:
        try:
            mentor = await db["mentors"].find_one({"_id": ObjectId(mentor_data.mentorId)})
        except:
            pass
            
        if not mentor:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Mentor not found or is not available")
...
async def remove_team_member(
    ...
    # 4. Remove the member
    result = await members_coll.delete_one({
        "teamId": team_id,
        "userId": user_id
    })
