        ...
        mock_disputes = [
            {
                ...
                "evidence": [
                    {
                        ...
                        "source": "GitHub",
                        "uploaded": "Aug 12, 10:21 AM",
                        "verified": True,
                    },
                    {
                        "id": "ev_2",
                        "type": "Plagiarism Analysis Report",
                        "source": "AI Scanner",
                        "uploaded": "Aug 12, 10:25 AM",
                        "verified": True,
                    },
                    {
                        "id": "ev_3",
                        "type": "Git Commit History Log",
                        "source": "GitHub API",
                        "uploaded": "Aug 12, 11:00 AM",
                        "verified": True,
                    },
                    {
                        "id": "ev_4",
                        "type": "Reporter Screenshot",
                        "source": "Upload",
                        "uploaded": "Aug 12, 10:12 AM",
                        "verified": False,
                    },
                ],
                "evidenceIntegrity": {
                ...
                "timeline": [
                    {
                        "date": "Aug 12, 10:12 AM",
                        "event": "Report received from John Doe",
                    },
                    {
                        "date": "Aug 12, 10:45 AM",
                        "event": "Investigator Dr. Kumar assigned",
                    },
                    {
                        "date": "Aug 12, 11:35 AM",
                        "event": "Evidence chain verified (94% similarity)",
                    },
                ],
                "communications": [
                    {
                        "author": "System",
                        "role": "Automated",
                        "text": "Plagiarism flag auto-triggered by AI evaluation.",
                        "isInternal": False,
                        "date": "Aug 12, 10:12 AM",
                    },
                    {
                        "author": "Admin Alex",
                        "role": "Internal Note",
                        "text": "Internal: Similarity confirmed against reference repo. Team explanation requested.",
                        "isInternal": True,
                        "date": "Aug 12, 11:40 AM",
                    },
                ],
            }
        ]
        ...
        result.append(
            {
                "id": str(d["_id"]),
                "disputeCode": d.get(
                    "disputeCode", f"DSP-2026-{str(d['_id'])[-5:].upper()}"
                ),
                ...
                "reporter": d.get(
                    ...
                    {
                        "name": "Anonymous",
                        "email": "reporter@hackzen.io",
                        "role": "Participant",
                    },
                ),
                "reportedTeam": d.get(
                    "reportedTeam",
                    {"name": "Target Team", "college": "Engineering Institution"},
                ),
                ...
                "assessment": (
                    ...
                    if assessment
                        ...
                        "evidenceVerified": "3 / 4",
                        "aiConfidence": 91,
                        "slaRemaining": sla_text,
                        "recommendation": "INVESTIGATE & REQUEST EXPLANATION",
                    }
                ),
                "similarityAnalysis": d.get(
                    "similarityAnalysis",
                    {
                        "overallSimilarity": 94,
                        "sourceCode": 97,
                ...
                "assignedInvestigator": d.get(
                    "assignedInvestigator",
                    {"name": "Admin User", "assignedAt": "Recently"},
                ),
...
async def assign_investigator(
    ...
    query = (
        ...
        if ObjectId.is_valid(dispute_id)
        else {"disputeCode": dispute_id}
    )

    dispute_doc = await db["disputes"].find_one(query)
    team_name = (
        dispute_doc.get("reportedTeam", {}).get("name", "") if dispute_doc else ""
    )
    ...
    return {
        "success": True,
        "message": f"Investigator {data.investigatorName} assigned to dispute.",
        "coi": coi_flag,
    }
    ...
    await db["disputes"].update_one(
        query,
        {
            ...
            "$push": {
                ...
                "timeline": {
                    "date": datetime.utcnow().strftime("%b %d, %I:%M %p"),
                    "event": f"Information requested from {data.target}",
                },
            },
        },
    )
...
async def resolve_dispute(
        ...
        query,
        {
            "$set": {
                ...
                "appeal": {
                    "eligible": True,
                    "windowHours": 48,
                    "status": "48-Hour Appeal Window Active",
                },
            },
            "$push": {
                "timeline": {
    ...
    return {
        "success": True,
        "message": "Dispute case resolved and audit trail recorded.",
    }
...
async def get_dispute_ai_assessment(
    dispute_id: str, current_user: dict = Depends(RequireRole(["admin", "superadmin"]))
):
    ...
    query = (
        {"_id": ObjectId(dispute_id)}
        if ObjectId.is_valid(dispute_id)
        else {"$or": [{"disputeCode": dispute_id}, {"id": dispute_id}]}
    )

    dispute = await db["disputes"].find_one(query)
    if not dispute and ObjectId.is_valid(dispute_id):
        dispute = await db["disputes"].find_one(
            {"$or": [{"disputeCode": dispute_id}, {"id": dispute_id}]}
        )
    ...
    if not dispute:
        dispute = {
            ...
            "evidence": [
                "Similarity in repository model code",
                "Commit timeline discrepancies",
            ],
        }
