...
async def get_my_hackathons_dashboard(
    ...
    memberships = await members_collection.find({"userId": current_user["sub"]}).to_list(100)
    ...
    for team in user_teams:
        ...
        role = next((m["role"] for m in memberships if m["teamId"] == team_id_str), "member")
...
async def get_organizer_stats(
    ...
    my_hackathons = await hackathons_collection.find({"organizerId": current_user["sub"]}).to_list(100)
    hackathon_ids = [str(h["_id"]) for h in my_hackathons]
    
    stats = {
        "totalHackathons": len(my_hackathons),
        "totalParticipants": await apps_collection.count_documents({"hackathonId": {"$in": hackathon_ids}}),
        "totalTeams": await teams_collection.count_documents({"hackathonId": {"$in": hackathon_ids}}),
        "hackathonBreakdown": []
    }
    ...
    for h in my_hackathons:
        ...
        stats["hackathonBreakdown"].append({
            ...
            "title": h["title"],
            "status": h["status"],
            "teamCount": await teams_collection.count_documents({"hackathonId": h_id}),
            "participantCount": await apps_collection.count_documents({"hackathonId": h_id})
        })
...
@router.put("/settings")
async def update_platform_settings(new_settings: dict, current_user: dict = Depends(RequireRole(["admin"]))):
    settings_collection = get_db()["settings"]
...
async def get_admins(current_user: dict = Depends(RequireRole(["admin"]))):
    users_collection = get_db()["users"]
    admins_cursor = users_collection.find({"role": {"$in": ["admin", "superadmin", "moderator"]}})
    ...
    for admin in admins_list:
        ...
        parts = name.split(' ')
        avatar = (parts[0][0] + parts[-1][0]).upper() if len(parts) > 1 else parts[0][:2].upper() if name else "AD"
...
async def remove_admin(admin_id: str, current_user: dict = Depends(RequireRole(["admin"]))):
...
async def get_platform_analytics(current_user: dict = Depends(RequireRole(["admin"]))):
    ...
    for i, month in enumerate(months):
        ...
        registration_data.append({"month": month, "value": val, "current": i == (datetime.now().month - 1)})

    # 3. Growth Data
    growth_data = [
        {"month": m, "value": val - 20, "current": d["current"]} 
        for m, val, d in zip(months, [r["value"] for r in registration_data], registration_data)
    ]

    # 4. Recent Reviews (from Evaluations)
    recent_evals = await eval_collection.find().sort("evaluatedAt", -1).limit(5).to_list(5)
    reviews = []
    for ev in recent_evals:
        ...
        reviews.append({
            ...
            "rating": min(5, (ev["totalScore"] // 20) + 1) if ev.get("totalScore") else 4,
    ...
    if not reviews:
        reviews = [
            { "id": "r1", "name": "System Auditor", "role": "Admin", "avatar": "https://ui-avatars.com/api/?name=Admin&background=3b82f6", "rating": 5, "feedback": "Platform stability is optimal. Monitoring tools active.", "time": "1d ago" }
        ]

    return {
        "metrics": {
            "newUsers": { "value": total_users, "growth": "+12%", "isPositive": True },
            "activeEntities": { "value": active_hackathons, "growth": "+5%", "isPositive": True },
            "satisfaction": { "value": 4.9, "subtitle": f"{len(reviews)} active reviews" }
        },
...
async def get_all_users(current_user: dict = Depends(RequireRole(["admin"]))):
    ...
    for u in users:
        ...
        result.append({
            ...
            "joinedDate": u["createdAt"].strftime("%b %d, %Y") if u.get("createdAt") else "N/A",
...
async def update_user_status(user_id: str, body: dict, current_user: dict = Depends(RequireRole(["admin"]))):
