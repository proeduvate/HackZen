...
@router.post("/", response_model=HackathonResponse, status_code=status.HTTP_201_CREATED, response_model_by_alias=False)
...
@router.get("/allHackathons", response_model=List[HackathonResponse], response_model_by_alias=False)
...
@router.get("/myhackathons", response_model=List[HackathonResponse], response_model_by_alias=False)
...
async def get_pending_hackathons(current_user: Dict[str, Any] = Depends(with_auth)):
    ...
    cursor = hackathons_col.find({"status": {"$in": ["Draft", "Pending", "Changes Requested"]}}).sort("createdAt", -1)
    hackathons = await cursor.to_list(100)

    result = []
    for h in hackathons:
        organizer = await users_col.find_one({"_id": ObjectId(h["organizerId"])}) if ObjectId.is_valid(h.get("organizerId", "")) else None
        ...
        result.append({
            ...
            "submittedOn": h["createdAt"].strftime("%b %d, %Y") if h.get("createdAt") else "N/A",
            "lastUpdated": "Recently",
            "lastUpdatedBy": "System",
            "changeItems": h.get("changeItems", []),
            "timeline": h.get("timeline", [
                {"id": 1, "status": "Submission Received", "timestamp": h["createdAt"].strftime("%b %d, %Y") if h.get("createdAt") else "N/A", "actor": None, "type": "secondary", "isActive": True}
            ])
        })
...
async def approve_hackathon(hackathon_id: str, current_user: Dict[str, Any] = Depends(with_auth)):
...
async def reject_hackathon(hackathon_id: str, current_user: Dict[str, Any] = Depends(with_auth)):
    """Reject a hackathon."""
    if current_user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
...
async def message_organizer(hackathon_id: str, body: Dict[str, Any], current_user: Dict[str, Any] = Depends(with_auth)):
