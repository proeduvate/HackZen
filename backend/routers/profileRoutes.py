...
async def update_my_profile_details(
    ...
    try:
        ...
        profile = await ProfileService.update_profile(str(current_user["_id"]), role, profile_dict, db)
        print(f"[DEBUG] Profile update result: {profile}")
        
        if not profile:
            raise HTTPException(status_code=404, detail="Profile not found or role invalid")
